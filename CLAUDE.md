...
3. **Props are the API contract.** Always declare an explicit `Props` interface (see `Button.svelte`, `CreateTaskForm.svelte`) rather than untyped `$props()`. Treat it like a function signature or DTO.
